...
void main()
{
    ...
    for( int i=0; i<lights.numLights; ++i )
    {
        ...
        if( !isDirectional )
        {
            float t = clamp( dot( frag.Position.xyz - lightStart, lights.uLight[i].horizontal ), 0.0, lights.uLight[i].width );
            vec3 lightPosition = lightStart + t * lights.uLight[i].horizontal;
            L = lightPosition - frag.Position.xyz; dist = length( L ); L /= dist;
        }
        ...
        if( hasShadows )
        {
            ...
            vec4 shadowCoord = lights.uLight[i].shadowMatrix * frag.Position;
        ...
        //			{
        //				vec4 modulationCoord = lights.uLight[i].modulationMatrix * frag.Position;
        ...
        if( !isDirectional )
        {
            vec3 lightPosition = calcRepresentativePoint( lightStart, lightEnd, frag.Position.xyz, -E );
            float scatter = calcScattering( frag.Position.xyz, lightPosition );
        ...
        if( isLinearLight ) {
            vec3 lightPosition = calcRepresentativePoint( lightStart, lightEnd, frag.Position.xyz, -reflect( E, N ) );
            L = lightPosition - frag.Position.xyz; dist = length( L ); L /= dist;
