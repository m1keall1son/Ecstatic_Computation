...
void main( void )
{
    ...
    vec3 rgbUL	= texture( uSampler, vTexCoord0 + vec2( -1.0, -1.0 ) * uPixel ).xyz;
    vec3 rgbUR	= texture( uSampler, vTexCoord0 + vec2(  1.0, -1.0 ) * uPixel ).xyz;
    vec3 rgbBL	= texture( uSampler, vTexCoord0 + vec2( -1.0,  1.0 ) * uPixel ).xyz;
    vec3 rgbBR	= texture( uSampler, vTexCoord0 + vec2(  1.0,  1.0 ) * uPixel ).xyz;
    vec3 rgbM	= texture( uSampler, vTexCoord0 ).xyz;
    ...
    vec3 color0 = 0.5 * (
                         texture( uSampler, vTexCoord0 + dir * ( 1.0 / 3.0 - 0.5 ) ).rgb +
                         texture( uSampler, vTexCoord0 + dir * ( 2.0 / 3.0 - 0.5 ) ).rgb );
    vec3 color1 = color0 * 0.5 + 0.25 * (
                                         texture( uSampler, vTexCoord0 + dir * -0.5 ).rgb +
                                         texture( uSampler, vTexCoord0 + dir *  0.5 ).rgb );
