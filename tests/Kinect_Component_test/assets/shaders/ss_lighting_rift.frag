...
vec4 getPosition( in vec2 uv, uint eye )
{
    ...
    mat4 uProjMatrixInverse = inverse( uRift.data.matrices[ 2*eye+1 ] );
    float nearClip = uRift.data.nearClip;
    float farClip = uRift.data.farClip;
...
void main(){
    ...
    uint eye = gl_FragCoord.x < .5 ? 0u : 1u;
