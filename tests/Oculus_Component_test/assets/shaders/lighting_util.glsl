                    ...
                    inout vec3 specular  )
{
    ...
    for( int i=0; i<lights.numLights; ++i )
    {
        ...
        if( hasShadows )
        {
            ...
            vec2 size = textureSize( uShadowMap, 0 )-vec2(1);
...
void calcLightingStereo( const in Material material,
                        const in Atmosphere atmosphere,
                        const in vec4 vertPosition,
                        const in vec3 vertNormal,
                        inout vec3 ambient,
                        ...
                        )
{
    ...
    for( int i=0; i<lights.numLights; ++i )
    {
        ...
        // Calculate end-points of the light for convenience.
        vec3 lightStart = lights.uLight[ind].position;
        vec3 lightEnd   = lights.uLight[ind].position + lights.uLight[ind].width * lights.uLight[ind].horizontal;
        vec3 lightColor = lights.uLight[ind].color.rgb * lights.uLight[ind].intensity;
        
        // Calculate direction and dist to light.
        float dist = 1.0;
        vec3  L = -lights.uLight[ind].direction;
